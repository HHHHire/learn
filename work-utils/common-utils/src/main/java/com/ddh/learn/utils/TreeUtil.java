...
public class TreeUtil {
    ...
    private static <T, ID extends Serializable> void processData(T t, List<T> otherList, Function<T, ID> idFunction, Function<T, ID> pidFunction, BiConsumer<T, List<T>> childrenConsumer) {
        if (otherList.size() == 0) {
            return;
        }
    ...
    private static <T, ID extends Serializable> void dataClassification(List<T> dataList, List<T> rootList, List<T> otherList, Function<T, ID> pidFunction, ID pidDefaultValue) {
        for (T t : dataList) {
